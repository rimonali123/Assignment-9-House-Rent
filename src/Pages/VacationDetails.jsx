...
const VacationDetails = () => {
    ...
    const { id5 } = useParams();
    const idInt = parseInt(id5);
    const vacation = vacations.find(vacation => vacation.id5 === idInt)
    console.log(vacation);
    const { image, estate_title, segment_name, description, price, status, area, location, facilities } = vacation;
    return (
        <div>
            <Helmet>
                <title>Vacation Rentals Card Details | Home Rent</title>
            </Helmet>
            ...
            <div className="card border-2 bg-base-100 shadow-xl mt-16 mb-10">
                ...
                <div className="px-5 py-5 space-y-2 items-center ">
                    ...
                    <h3 className="text-lg "><span className="font-bold">Card Id:</span> {id5}</h3>
                    ...
                    <div className=' flex flex-col gap-2'><span className="font-bold">Facilities:</span>
                        <span className="ml-5">
                            {
                                facilities.map(facilitie => (<li key={facilitie}>{facilitie}</li>))
                            }
                        </span>
                    </div>
                </div>
