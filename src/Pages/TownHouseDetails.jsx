...
const TownHouseDetails = () => {
    const towns = useLoaderData();
    const { id1 } = useParams();
    const idInt = parseInt(id1);
    const town = towns.find(town => town.id1 === idInt)
    console.log(town);
    const { image, estate_title, segment_name, description, price, status, area, location, facilities } = town;
    return (
        <div>
            <Helmet>
                <title>TownHouse Card Details | Home Rent</title>
            </Helmet>
            ...
            <div className="card border-2 bg-base-100 shadow-xl mt-16 mb-10">
                ...
                <div className="px-5 py-5 space-y-2 items-center ">
                    ...
                    <h3 className="text-lg "><span className="font-bold">Card Id:</span> {id1}</h3>
