...
const SeniorCommunityCard = ({item}) => {
    const { id4, image, estate_title, description, price, status, facilities } = item;
    return (

        <div data-aos="zoom-in"  data-aos-duration="1500" className="card hover:border-2 hover:scale-105 bg-base-100 shadow-xl">
            <figure className="px-5 pt-5">
                <img src={image} alt="image" className="rounded-xl w-full h-52" />
            </figure>
            <div className="px-5 py-5 space-y-2 items-center ">
                ...
                <div className="card-actions">
                    <Link to={`/seniorCommunityDetails/${id4}`} ><button className="btn btn-primary w-full text-xl font-bold hover:btn-secondary  ">View Property</button></Link>
                </div>
            </div>
        </div>
...
SeniorCommunityCard.propTypes = {
    item: PropTypes.object.isRequired
  };

export default SeniorCommunityCard;